<template>
  <q-page class="bg-grey-2 q-pa-lg">
    <div class="row items-center justify-between q-mb-xl">
      <div class="col-auto">
        <q-btn flat color="primary" icon="arrow_back" @click="index" />
      </div>
    ...
    <div class="text-center text-h3 text-weight-bolder q-mb-lg text-blue-grey-9">
      ...
      >
        <q-card class="my-card cursor-pointer q-hoverable" @click="abrirDetalle(procedimiento)">
          ...
          <q-card-section class="text-center bg-primary text-white">
            <q-icon name="construction" size="2.5rem" />
    ...
    <div v-if="filteredProcedimientos.length === 0" class="text-center q-pa-xl text-grey-6">
      <q-icon name="search_off" size="5rem" />
      <div class="text-h6">No encontramos procedimientos con ese nombre</div>
    </div>
  ...
  >
    <q-card class="bg-grey-1">
      <q-bar class="bg-primary text-white q-pa-lg">
        ...
        <q-space />
        <q-btn
          v-if="authStore.usuario?.edit_procedimientos"
          flat
          round
          icon="edit"
          @click="PrepararEdit(procedimientoSeleccionado)"
        >
  ...
  <q-dialog v-model="NuevoProcedimiento" persistent>
    <q-card
      style="width: 100%; max-width: 900px; display: flex; flex-direction: column"
      class="col"
    >
      ...
      <q-stepper class="q-pa-none" v-model="step" header-nav color="primary" animated flat>
      ...
      <q-card-actions align="right" class="q-pa-sm bg-white">
        <q-btn flat label="Cancelar" color="negative" v-close-popup class="q-mr-sm" />

        <q-space />
        <!-- Esto empuja los de navegación a la derecha -->
...
const EditarProcedimiento = async () => {
  try {
    ...
    const response = await api.put(
      `/api/procedimientos/${formProcedimiento.value.ProceId}`,
      formProcedimiento.value,
    )
...
.bg-blue-grey-1 {
  border-bottom: 2px solid #e0e0e0; /* Una línea sutil para separar el título del contenido */
...
.border-amber {
  border: 1px solid #ffc107; /* Resalta más la advertencia */
}
</style>
